...
int main(int argc,char**argv){
    if(argc<7){
        printf("%s <dev> dspec dtype doper ttype endir",argv[0]);
        return 0;
    }
    ...
    int ttype = atoi(argv[5]); 
    int endir= atoi(argv[6]);
...
*/
    ...
    xnvme_ctx->cmd.common.opcode=XNVME_SPEC_NVM_OPC_IO_MGMT_SEND;
	...
	switch (dtype) {
        case NVME_DIR_IDENTIFY:
			...
			// dw12 = ttype << 8 | cfg.endir;
            xnvme_ctx->cmd.common.cdw12 = ttype << 8 | endir;
                        break;
                default:
                        fprintf(stderr, "invalid directive operations for Identify Directives\n");
                        // return EINVAL;
