...
int main(int argc, char **argv)
{
    ...
    if (err < 0)
        printf("Failed to drain xNVMe queue\n");

    // printf("ruh_status.nruhsd %d /// fr ratio %u copied page %u block erase %u\n", ruh_status.nruhsd, ruh_status.free_space_ratio, ruh_status.copied_page, ruh_status.block_erased);
    ...
    for (int i = 0; i < ruh_status.nruhsd; i++)
    {
        ...
        */
        printf("pid: (%d) ruhid: (%d) earutr: (%d) ruamw: (%ld)\n",
