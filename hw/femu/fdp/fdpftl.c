...
static uint64_t gc_write_page(struct ssd *ssd, struct ppa *old_ppa, int stream_id, int rg_id, bool to_other_rg)
{
    ...
    /* Track media bytes written by GC (media overhead) */
    if (ssd->fdp.enabled) {
        ssd->fdp.stats.mbmw++;  /* One page of media writes by GC */
    }
...
static int do_gc(struct ssd *ssd, bool force, int rg_id)
{
    ...
    ftl_log("[%s]GC-ing line:%d,ipc=%d,victim=%d,full=%d,free=%d,stream_id=%d,rg_id=%d, discard %lu read/write %lu/%lu block_erased %lu copied %lu\n", asctime(tm_info), ppa.g.blk,
...
static uint64_t msssd_io_mgmt_recv_ruhs(struct ssd *ssd, NvmeRequest *req, size_t len)
{
    ...
    for (ph = 0; ph < ssd->handle_number; ph++)
    {
        for (rg = 0; rg < ssd->rg_number; rg++, ruhsd++)
        {
            ...
            ftl_debug("ruhsd->ruhid %u\n", ruhsd->ruhid);
            ftl_debug("ruhsd->pid %u\n", ruhsd->pid);
        }
    }
...
/* Read-only stats: print statistics without resetting counters */
/* Print SSD statistics - shared by readonly and reset functions */
static void print_ssd_statistics(struct ssd *ssd, bool will_reset)
{
    int i, j;
    struct ssdparams *spp = &ssd->sp;
    time_t t;
    ...
    ftl_log("\n");
    if (will_reset) {
        ftl_log("============== FDP Statistics (Before Reset) ==============\n");
    } else {
        ftl_log("============== FDP Statistics (Read-Only) ==============\n");
    }
    ftl_log("Timestamp: %s", asctime(tm_info));
    ftl_log("=========================================================\n");

    /* Print SSD parameters */
    ftl_log("SSD Configuration:\n");
    ftl_log("  Channels: %d, LUNs/Ch: %d, Planes/LUN: %d\n",
            spp->nchs, spp->luns_per_ch, spp->pls_per_lun);
    ftl_log("  Blocks/Plane: %d, Pages/Block: %d\n",
            spp->blks_per_pl, spp->pgs_per_blk);
    ftl_log("  Pages/Line: %d, Blocks/Line: %d\n",
            spp->pgs_per_line, spp->blks_per_line);
    ftl_log("  RG count: %d, Handle count: %d\n",
            ssd->rg_number, ssd->handle_number);

    /* Print I/O statistics */
    ftl_log("\nI/O Statistics:\n");
    ftl_log("  Write IOs: %lu\n", ssd->fs_stat.write_io_n);
    ftl_log("  Read IOs: %lu\n", ssd->fs_stat.read_io_n);
    ftl_log("  Read sum: %lu\n", ssd->fs_stat.read_sum);

    /* Print GC statistics */
    ftl_log("\nGC Statistics:\n");
    ftl_log("  Blocks erased: %lu\n", ssd->fs_stat.block_erased);
    ftl_log("  Pages copied (GC): %lu\n", ssd->fs_stat.copied);
    ftl_log("  Pages invalidated: %lu\n", ssd->fs_stat.invalidated);
    ftl_log("  Discards: %lu (ignored: %lu)\n",
            ssd->fs_stat.discard, ssd->fs_stat.discard_ignored);

    /* Print per-RG line statistics */
    ftl_log("\nPer-RG Line Statistics:\n");
    for (i = 0; i < ssd->rg_number; i++)
    {
        struct line_mgmt *lm = &ssd->lm[i];
        ftl_log("  RG %d: free=%d, full=%d, victim=%d\n",
                i, lm->free_line_cnt, lm->full_line_cnt, lm->victim_line_cnt);
    }

    /* Print per-handle write pointer statistics */
    ftl_log("\nWrite Pointer Statistics (written pages):\n");
    for (i = 0; i < ssd->handle_number; i++)
    {
        ftl_log("  Handle %d:", i);
        for (j = 0; j < ssd->rg_number; j++)
        {
            struct write_pointer *wpp = &ssd->wp[i][j];
            total_written += wpp->written;
            ftl_log(" RG%d=%lu", j, wpp->written);
        }
        ftl_log("\n");
    }

    /* Calculate and print WAF if possible */
    if (total_written > 0)
    {
        uint64_t copied = ssd->fs_stat.copied;
        /* WAF = 1 + (copied pages / host written pages) */
        uint64_t waf_int = 1 + (copied / total_written);
        uint64_t waf_frac = ((copied % total_written) * 100) / total_written;
        ftl_log("\nWrite Amplification:\n");
        ftl_log("  Host written: %lu pages\n", total_written);
        ftl_log("  GC copied: %lu pages\n", copied);
        ftl_log("  WAF: %lu.%02lu (1 + %lu/%lu)\n",
                waf_int, waf_frac, copied, total_written);
    }

    /* Print FDP-specific stats if enabled */
    if (ssd->fdp.enabled)
    {
        ftl_log("\nFDP Statistics:\n");
        ftl_log("  Host bytes mapped to writes (hbmw): %lu\n", ssd->fdp.stats.hbmw);
        ftl_log("  Media bytes written (mbmw): %lu\n", ssd->fdp.stats.mbmw);
        ftl_log("  Media bytes erased (mbe): %lu\n", ssd->fdp.stats.mbe);
        ftl_log("  Host events logged: %u\n", ssd->fdp.host_events.nelems);
        ftl_log("  Controller events logged: %u\n", ssd->fdp.ctrl_events.nelems);
    }

    ftl_log("=========================================================\n");
    if (will_reset) {
        ftl_log("NOTE: Counters will be RESET after this\n");
    } else {
        ftl_log("NOTE: Counters NOT reset (read-only mode)\n");
    }
    ftl_log("=========================================================\n\n");
...
/* Read-only stats: print statistics without resetting counters */
static uint64_t msssd_io_mgmt_send_sungjin_readonly(struct ssd *ssd, NvmeRequest *req)
{
    (void)req;
    print_ssd_statistics(ssd, false);
    return 0;
}
...
static uint64_t msssd_io_mgmt_send_sungjin(struct ssd *ssd, NvmeRequest *req)
{
    ...
    print_ssd_statistics(ssd, true);

    ftl_debug("msssd_io_mgmt_send_sungjin: Performing reset...\n");
    ftl_debug("  blks_per_line: %d, pgs_per_line: %d\n", spp->blks_per_line, spp->pgs_per_line);
    ...
    if (sum_written)
    {
        ftl_debug(((sum_written + ssd->fs_stat.copied) * 100) / sum_written);
    }
...
static uint64_t msssd_io_mgmt_send(struct ssd *ssd, NvmeRequest *req)
{
    ...
    printf("msssd_io_mgmt_send: cdw10=0x%x, mo=0x%x\n", cdw10, mo);
    ...
    case NVME_IOMS_MO_RUH_UPDATE:
        /* Parse PIDs from the request and update RUHs */
        {
            NvmeRwCmd *rw = (NvmeRwCmd *)cmd;
            uint16_t pid = le16_to_cpu(rw->dspec);
            uint32_t nsid = le32_to_cpu(cmd->nsid);
            if (fdpssd_update_ruh(ssd, pid, nsid)) {
                return NVME_SUCCESS;
            }
            return NVME_INVALID_FIELD | NVME_DNR;
        }
    case NVME_IOMS_MO_SUNGJIN:
        printf("msssd_io_mgmt_send: calling msssd_io_mgmt_send_sungjin\n");
        return msssd_io_mgmt_send_sungjin(ssd, req);
    case NVME_IOMS_MO_SUNGJIN_READONLY:
        printf("msssd_io_mgmt_send: calling msssd_io_mgmt_send_sungjin_readonly\n");
        return msssd_io_mgmt_send_sungjin_readonly(ssd, req);
    default:
        printf("msssd_io_mgmt_send: unknown MO=0x%x\n", mo);
        return NVME_INVALID_FIELD | NVME_DNR;
    };
}
...
/* Get current timestamp in 100ns units (NVMe timestamp format) */
static uint64_t fdp_get_timestamp(void)
{
    return qemu_clock_get_ns(QEMU_CLOCK_REALTIME) / 100;
}

/* Allocate a new event in the circular event buffer */
static struct fdp_event *fdp_alloc_event(struct fdp_event_buffer *ebuf)
{
    struct fdp_event *ret = NULL;
    bool is_full = (ebuf->next == ebuf->start) && ebuf->nelems;

    ret = &ebuf->events[ebuf->next++];
    if (ebuf->next == FDP_MAX_EVENTS) {
        ebuf->next = 0;
    }
    if (is_full) {
    ...
    memset(ret, 0, sizeof(struct fdp_event));
    ret->timestamp = fdp_get_timestamp();

    return ret;
}

/* Check if event should be logged based on RUH event filter */
static inline bool fdp_should_log_event(struct fdp_ruh *ruh, uint8_t event_type)
{
    /* Event filter bit positions (from NVMe spec) */
    static const uint8_t evf_shifts[FDP_EVT_MAX] = {
    ...
    if (event_type >= FDP_EVT_MAX) {
        return false;
    }
    return (ruh->event_filter >> evf_shifts[event_type]) & 0x1;
}

/* Initialize FDP configuration */
void fdpssd_init_fdp(struct ssd *ssd, uint16_t nruh, uint16_t nrg, uint64_t runs)
{
    struct fdp_config *fdp = &ssd->fdp;
    int i;

    fdp->enabled = true;
    fdp->nruh = nruh;
    fdp->nrg = nrg;
    fdp->rgif = 0;  /* No reclaim group identifier format by default */
    fdp->runs = runs;

    /* Allocate RUHs */
    fdp->ruhs = g_malloc0(sizeof(struct fdp_ruh) * nruh);
    for (i = 0; i < nruh; i++) {
        fdp->ruhs[i].ruht = 0;  /* Initially Isolated */
        fdp->ruhs[i].ruha = 0;
        fdp->ruhs[i].event_filter = 0xFFFFFFFFFFFFFFFFULL; /* Log all events by default */
        fdp->ruhs[i].ruamw = runs;

        /* Allocate RUs for each RUH */
        fdp->ruhs[i].rus = g_malloc0(sizeof(struct fdp_ru) * nrg);
        for (int rg = 0; rg < nrg; rg++) {
            fdp->ruhs[i].rus[rg].ruamw = runs;
        }
    }

    /* Initialize event buffers */
    memset(&fdp->host_events, 0, sizeof(struct fdp_event_buffer));
    memset(&fdp->ctrl_events, 0, sizeof(struct fdp_event_buffer));
    ...
    ftl_log("FDP initialized: nruh=%u, nrg=%u, runs=%lu\n", nruh, nrg, runs);
}
                      ...
                      uint32_t nsid, uint16_t rgid, uint16_t ruhid, bool host_event)
{
    ...
    /* Get the RUH to check event filter */
    if (ruhid < fdp->nruh) {
        ruh = &fdp->ruhs[ruhid];
        if (!fdp_should_log_event(ruh, event_type)) {
            return;
        }
    }

    /* Select appropriate event buffer */
    ebuf = host_event ? &fdp->host_events : &fdp->ctrl_events;

    /* Allocate and fill event */
    e = fdp_alloc_event(ebuf);
    e->type = event_type;
    e->flags = 0;
    ...
    if (nsid != 0) {
        e->flags |= FDPEF_NSIDV;
        e->nsid = cpu_to_le32(nsid);
    }
    if (rgid != 0xFFFF || ruhid != 0xFFFF) {
        e->flags |= FDPEF_LV;
        e->rgid = cpu_to_le16(rgid);
        e->ruhid = cpu_to_le16(ruhid);
    }

    ftl_debug("FDP Event logged: type=%u, pid=%u, nsid=%u, rgid=%u, ruhid=%u\n",
              event_type, pid, nsid, rgid, ruhid);
}

/* Update Reclaim Unit Handle - triggers RU change */
bool fdpssd_update_ruh(struct ssd *ssd, uint16_t pid, uint32_t nsid)
{
    ...
    if (!fdp->enabled) {
        return true;  /* If FDP not enabled, succeed silently */
    }

    /* Parse PID into placement handle and reclaim group */
    if (fdp->rgif == 0) {
        ph = pid;
        rg = 0;
    } else {
        ph = pid & ((1 << (15 - fdp->rgif)) - 1);
        rg = pid >> (16 - fdp->rgif);
    }

    /* Validate */
    if (ph >= fdp->nruh || rg >= fdp->nrg) {
        /* Log invalid PID event */
        fdpssd_log_event(ssd, FDP_EVT_INVALID_PID, pid, nsid, rg, ph, true);
        return false;
    }
    ...
    /* Check if RU has remaining media writes */
    if (ru->ruamw > 0) {
        /* Log RU not fully written event */
        fdpssd_log_event(ssd, FDP_EVT_RU_NOT_FULLY_WRITTEN, pid, nsid, rg, ph, true);

        /* Account for wasted media capacity */
        fdp->stats.mbmw += ru->ruamw;
    }

    /* Reset RUAMW for the new RU */
    ru->ruamw = ruh->ruamw;

    ftl_debug("FDP RUH updated: pid=%u (ph=%u, rg=%u), new ruamw=%lu\n",
              pid, ph, rg, ru->ruamw);

    return true;
}

/* Get FDP event log - implements IO Management Receive for events */
uint64_t fdpssd_get_event_log(struct ssd *ssd, NvmeRequest *req)
{
    return msssd_io_mgmt_recv(ssd, req);
}

/* Handle IO Management Send - RUH Update */
uint64_t fdpssd_handle_update(struct ssd *ssd, NvmeRequest *req)
{
    return msssd_io_mgmt_send(ssd, req);
}
...
static uint64_t fdpssd_write(struct ssd *ssd, NvmeRequest *req)
{
    ...
    /* Log FDP invalid PID event if detected */
    if (invalid_pid && ssd->fdp.enabled) {
        uint32_t nsid = le32_to_cpu(req->cmd.nsid);
        fdpssd_log_event(ssd, FDP_EVT_INVALID_PID, pid, nsid, rg_id, stream_id, true);
    }

    /* Track host bytes written for FDP stats */
    if (ssd->fdp.enabled) {
        ssd->fdp.stats.hbmw += (len + 1);  /* Host bytes mapped to media writes */
    }
